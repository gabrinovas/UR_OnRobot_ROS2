...
public:
  ...
  // Métodos de acceso para testing unitario
  WatchdogState getState() const { return state_; }
  uint8_t getSafetyMode() const { return current_safety_mode_; }
  bool isFailSafeHoldEngaged() const { return fail_safe_hold_engaged_; }
  size_t getProtectiveStopCount() const { return protective_stop_count_; }
  size_t getEstopCount() const { return estop_count_; }
  size_t getRecoveryCount() const { return recovery_count_; }

  void triggerSafetyMode(uint8_t mode)
  {
    ur_dashboard_msgs::msg::SafetyMode mock_msg;
    mock_msg.mode = mode;
    safetyModeCallback(std::make_shared<ur_dashboard_msgs::msg::SafetyMode>(mock_msg));
  ...
  void triggerRobotMode(int8_t mode)
  {
    ur_dashboard_msgs::msg::RobotMode mock_msg;
    mock_msg.mode = mode;
    robotModeCallback(std::make_shared<ur_dashboard_msgs::msg::RobotMode>(mock_msg));
  }
  ...
  void produceDiagnostics(diagnostic_updater::DiagnosticStatusWrapper & stat)
  {
    ...
    } else if (has_telemetry_ && telemetry_age > 3.0) {
      stat.summary(diagnostic_msgs::msg::DiagnosticStatus::WARN,
